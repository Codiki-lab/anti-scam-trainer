...
- `POST /api/v1/training/free-play/start?role=...` — начать или продолжить Свободную игру для выбранной роли. Новый режим доступен после прохождения Уровня 4 хотя бы на одну Звезду во всех шести Темах этой Ролевой ветки; до этого возвращается `CONTENT_UNAVAILABLE`.
...
- Result также содержит один компактный `feedback` (причина, не более двух сигналов и безопасная альтернатива) и может содержать необязательный `micro_question`. `POST /api/v1/attempts/{id}/micro-question/answer` проверяет ответ, но не меняет Балл, Звёзды, Прогресс, Quiz или Серию дней.
...
- `POST /api/v1/integrations/avito-chat/recommendations` доступен только аутентифицированному Пользователю и принимает `source=avito_chat_demo`, Ролевую ветку, 2–6 обезличенных Сообщений, `risk_type` и до трёх `risk_signals`. Идентификатор Пользователя в body не принимается.
- Снимок диалога не записывается. URL, телефоны, реквизиты, коды подтверждения и идентификаторы чата/объявления отклоняются с `400 VALIDATION_ERROR` до подбора Темы.
- Сервер возвращает только опубликованную Тему той же Ролевой ветки, её публичные поля, безопасное `explanation`, `next_action` и `fallback`. Риск не сопоставляется на frontend; policy, rubric, модельный контекст и оценки модели не возвращаются.
- Точные server-side соответствия поддерживают `phishing`, `prepayment`, `delivery`, `external_messenger`, `account_takeover`, `fake_payment` и `social_engineering` для обеих Ролевых веток. Неизвестный риск либо недоступность целевой Темы получает `fallback=true` и общую опубликованную Тему безопасной сделки соответствующей Ролевой ветки.
- Подбор детерминированный, не вызывает Ollama и имеет независимый bounded rate limit на Пользователя. `429 RATE_LIMITED` содержит `Retry-After`.
