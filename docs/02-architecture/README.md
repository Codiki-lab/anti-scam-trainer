...
Сервис каждого модуля зависит от малого интерфейса своего репозитория. Общие модули находятся в `internal/core`: предметные правила — в `domain`, а технические адаптеры — в `aiprovider`, `config`, `postgres`, `errors` и `transport`. Например, допустимые переходы статуса прохождения и расчёт звёзд по баллу остаются чистыми правилами в `internal/core/domain`.
