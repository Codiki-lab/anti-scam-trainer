...
Миграция `000012_stable_dashboard_recommendations` хранит выбранный `continue_action` по ключу `(user_id, activity_date, user_role)`. Рекомендация остаётся устойчивой в течение московской календарной даты и пересчитывается после выполнения соответствующей учебной активности.
