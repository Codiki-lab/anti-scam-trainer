...
Миграция `000016_polished_demo_dialogues` убирает механические префиксы из готовых Ответов пользователя Уровней 2–3 и заменяет универсальные реакции Виртуального собеседника на тематические реплики для всех 12 Тем. Она изменяет только опубликованный штатный seed-контент `issue-103-complete`, сохраняет исходные значения в snapshot-таблице для точного rollback и не затрагивает пользовательские Сценарии или исторические Сообщения.
