...
`daily_tasks` хранит одно назначение по ключу `(user_id, activity_date, user_role)`. Запись содержит действие из словаря `ContinueAction`, целевые `topic_id`, `level_number` или `attempt_id` и необязательное `completed_at`. Дата определяется по `Europe/Moscow`. Выполнение Теории, успешного Quiz, Уровня со Звездой и Свободной игры обновляет назначение внутри той же PostgreSQL-транзакции, что и соответствующее учебное действие и Серия дней.
