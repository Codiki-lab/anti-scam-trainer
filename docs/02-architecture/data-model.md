...
Для одной пары «Пользователь — ролевая ветка» может существовать только одна незавершённая Свободная игра. Скрытый `is_scam` хранится в Прохождении, не входит в незавершённый GameState и раскрывается только в Result. Восемь опубликованных Сценариев миграции 5 покрывали buyer/seller × Уровни 1–4; AI-инструкции, fallback-реплики и рубрики остаются управляемым контентом.
...
- Роль пользователя ограничена `buyer` и `seller`; тип ответа — `multiple_choice`, `similar_choice`, `mixed` или `free_text`. В новой границе evaluator и generator независимы, а сервер владеет фазой, Баллами и завершением.
