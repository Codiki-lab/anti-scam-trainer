...
Для одной пары «Пользователь — ролевая ветка» может существовать только одна незавершённая Свободная игра. Скрытый `is_scam` хранится в Прохождении, но не входит в игровой HTTP DTO. Восемь опубликованных Сценариев покрывают buyer/seller × Уровни 1–4; AI-инструкции, fallback-реплики и рубрики остаются управляемым контентом.
