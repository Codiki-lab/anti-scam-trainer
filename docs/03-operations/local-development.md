...
Команда пересобирает и запускает контейнер API. Перед его стартом одноразовый контейнер `anti-scam-trainer-migrate` применяет все ещё не применённые миграции из `backend/migrations`; API ждёт его успешного завершения. Поэтому чистый запуск создаёт схему автоматически, а изменения кода и новые миграции применяются при следующем `make up`.
