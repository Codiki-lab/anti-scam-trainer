...
Одновременно сервер случайно выбирает один из разрешённых товарных контекстов соответствующей ролевой ветки и закрепляет его за Прохождением. Поэтому новая Свободная игра может быть посвящена разным товарам и способам получения, но повторное открытие той же попытки не меняет предмет сделки. Стартовая фаза `hook` используется только для первой реплики; после каждого Ответа пользователя сервер последовательно переводит диалог в `escalation`, `critical_request` и `resolution`, поэтому собеседник не начинает разговор заново.
...
| Уровень 4 | ScammerGenerator создаёт реплику, отдельный Evaluator оценивает Ответ пользователя |
| Свободная игра | Generator ведёт разговор выбранного типа, отдельный Evaluator оценивает Ответ пользователя |

Модель не определяет Баллы, Звёзды, фазы, переходы, завершение или Прогресс. Evaluator возвращает `score` 1–4, который сервер отображает в 0/25/75/100; generator возвращает только `message`, `tactic` и установленную сервером `phase`.

Оба результата проходят строгую JSON/policy-валидацию. Неизвестные или отсутствующие обязательные поля, лишний JSON, URL, телефоны, карточные данные, неверные enum и выход за размер отклоняются до записи состояния. После одной repair-попытки повторно невалидный evaluator заменяется детерминированной консервативной оценкой: явное безопасное действие получает частичный безопасный результат, автоматическое согласие или рискованное действие — 0 Баллов, неоднозначный ответ — промежуточный результат. Generator использует ещё не показанную разрешённую fallback-реплику. Generator получает явные роли Пользователя и собеседника и выбирает одну из разрешённых сервером реплик текущей фазы; их список входит в JSON schema, поэтому повтор реплики из истории, смена роли, произвольное местоположение, состояние товара или совершённое действие пройти проверку не могут.
