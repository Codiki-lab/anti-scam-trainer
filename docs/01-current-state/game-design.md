...
Для Уровня 4 и Свободной игры `finish=true` допустим в третьем и последующих свободных Ответах пользователя. Диалог принимает не более шести свободных Ответов. Сервер управляет фазами `hook → escalation → critical_request → resolution`, завершает Прохождение по безопасному выходу, критически опасному действию, явному завершению или пределу ходов и хранит compact summary после четвёртой пары. При транспортной либо неустранённой evaluator-ошибке Ответ, Сообщения, фаза, счётчик, Балл и Прогресс не записываются.
...
Оба результата проходят строгую JSON/policy-валидацию. Неизвестные или отсутствующие обязательные поля, лишний JSON, URL, телефоны, карточные данные, неверные enum и выход за размер отклоняются до записи состояния. Generator выбирает одну из разрешённых сервером реплик текущей фазы; их список входит в JSON schema, поэтому произвольное местоположение, состояние товара или совершённое действие пройти проверку не может. Допускается одна repair-попытка.
