...
Оба результата проходят строгую JSON/policy-валидацию. Неизвестные или отсутствующие обязательные поля, лишний JSON, URL, телефоны, карточные данные, неверные enum и выход за размер отклоняются до записи состояния. После одной repair-попытки повторно невалидный evaluator заменяется консервативной оценкой без Баллов; generator использует разрешённую fallback-реплику. Generator выбирает одну из разрешённых сервером реплик текущей фазы; их список входит в JSON schema, поэтому произвольное местоположение, состояние товара или совершённое действие пройти проверку не может.
