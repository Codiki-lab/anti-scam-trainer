...
В интерфейсе готовый вариант показывается как возможная реплика Пользователя от первого лица и отправляется одним нажатием. Отдельного режима «выбрать действие и подтвердить» нет. На `free_text`-Шаге вместо вариантов показывается поле, в котором Пользователь пишет собственную реплику; на одном Шаге эти способы ответа не смешиваются.
...
- На карточке Темы круговой индикатор показывает количество пройденных Уровней из четырёх; на карточке отдельного Уровня показываются его Звёзды.
