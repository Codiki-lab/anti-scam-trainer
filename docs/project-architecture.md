...
3. GameState возвращает название и описание Сценария, Тему, Уровень, обе роли, типизированный `product_context`, полный `step_progress`, видимую реплику, историю и структурированные сохранённые Ответы, но не раскрывает внутренние инструкции и правильность ответа.
...
7. `daily_task` в Dashboard является сохранённым для московской даты Снимком диалога; frontend отправляет только бинарный ответ и не вычисляет вердикт.
8. `product_context` имеет обязательные `item_title`, `category`, `deal_method` и опциональные `price`, `currency=RUB`, `location`, контролируемый `image_key`; frontend валидирует эту схему на HTTP-границе.
...
- L4 и Свободная игра входят в реализованный MVP, используют управляемую последовательность `hook → escalation → critical_request` и не более пяти свободных Ответов пользователя; после двух Ответов Пользователь может завершить Прохождение досрочно;
- structured output проходит schema и policy validation;
- транспортная ошибка модели не меняет состояние и запрос можно повторить; повторно невалидный evaluator использует консервативную fallback-оценку без Баллов, а Сценарий хранит безопасную fallback-реплику.
